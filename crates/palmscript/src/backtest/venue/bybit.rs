...
pub(super) fn validate_spot(order: &OrderDecl) -> Result<(), String> {
    validate_spot_like_order(order, "Bybit spot")
}

pub(super) fn validate_usdt_perps(order: &OrderDecl) -> Result<(), String> {
    validate_usdt_perps_like_order(order, "Bybit USDT perps")
}
