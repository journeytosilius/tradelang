...
impl<'a> LexerState<'a> {
    ...
    fn lex(&mut self) -> Result<Vec<Token>, CompileError> {
        while let Some(ch) = self.peek_char() {
            match ch {
                ...
                ':' => self.push_single(TokenKind::Colon, false, false),
                '?' => self.push_single(TokenKind::Question, false, false),
    ...
    fn lex_ident(&mut self) {
        ...
        let kind = match text.as_str() {
            ...
            "arb_entry" => TokenKind::ArbEntry,
            "arb_exit" => TokenKind::ArbExit,
            "arb_order" => TokenKind::ArbOrder,
            "transfer" => TokenKind::Transfer,
            "cooldown" => TokenKind::Cooldown,
            "max_bars_in_trade" => TokenKind::MaxBarsInTrade,
            "portfolio_group" => TokenKind::PortfolioGroup,
            "max_positions" => TokenKind::MaxPositions,
            "max_long_positions" => TokenKind::MaxLongPositions,
            "max_short_positions" => TokenKind::MaxShortPositions,
            "max_gross_exposure_pct" => TokenKind::MaxGrossExposurePct,
            "max_net_exposure_pct" => TokenKind::MaxNetExposurePct,
