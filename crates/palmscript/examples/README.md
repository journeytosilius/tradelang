...
- `crates/palmscript/examples/strategies/experimental/binance_usdm_top10_dynamic_regime_reversion.ps`: experimental 15m dynamic-routing Binance USD-M top-10 major-coin basket that uses `current_execution()` plus `venue = current_execution()` to trade BTC, ETH, SOL, XRP, DOGE, BNB, ADA, LINK, AVAX, and DOT from one portfolio script
...
./palmscript run backtest crates/palmscript/examples/strategies/experimental/ada_usdm_regime_scalper.ps --from 1771286400000 --to 1773878400000 --maker-fee-bps 2 --taker-fee-bps 5 --slippage-bps 1 --max-volume-fill-pct 0.10 --leverage 2
