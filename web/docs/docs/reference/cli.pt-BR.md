...
- se um desses endpoints auxiliares da Binance USD-M nao retornar linhas para a janela solicitada, a sessao paper agora continua armando e expõe esse campo como `na` em vez de falhar a sessao inteira
...
- `paper-status` e `paper-export` agora incluem contadores de readiness e um inventario `required_feeds` com estado de armamento, readiness, horario da ultima barra fechada, mensagens de falha por feed e snapshots de cotacao para cada alias de execucao
