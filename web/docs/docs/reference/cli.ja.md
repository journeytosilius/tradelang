...
- `paper-status` と `paper-export` には、feed readiness の集計に加えて、各 execution alias ごとの arm 状態、readiness、最新の確定バー時刻、feed ごとの失敗メッセージ、quote snapshot を含む `required_feeds` インベントリも含まれる
