...
Les declarations `module` sont reservees au niveau superieur. En v1 elles ne
nomment que les roles `entry`, `entry2` et `entry3` pour les diagnostics.
...
- `venue = <execution_alias_expr>` binds an order role to a declared execution target.
