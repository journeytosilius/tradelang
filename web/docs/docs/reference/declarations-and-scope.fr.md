...
- `last_*_exit.kind` inclut `exit_kind.liquidation` en plus des types de
  sortie existants
- les noms de trigger reserves comme `trigger long_entry = ...` ne sont plus
  des alias executables ; utilisez des declarations `entry` / `exit` de
  premiere classe avec des templates `order ...` correspondants
...
- These declarations are top-level only, and portfolio caps now evaluate once per bar from literal or runtime numeric expressions.
...
- Order constructors now accept named arguments, and `venue = exec` or `venue = current_execution()` binds that order role to a declared execution alias.
