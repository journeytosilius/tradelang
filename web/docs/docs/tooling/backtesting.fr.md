...
Les scripts de trading peuvent aussi etiqueter des roles d'entree pour une
attribution par module :

- `module breakout = entry long`
- `module pullback = entry2 long`

Ces etiquettes apparaissent dans les diagnostics comme `entry_module` et
`by_entry_module`.
...
Historical exchange downloads for backtest-oriented CLI runs are cached on disk automatically. PalmScript reuses overlapping windows across backtest, walk-forward, walk-forward-sweep, and optimize runs instead of refetching the same bars every invocation. The cache only marks contiguous bars it actually received as covered, so incomplete live-edge windows are retried automatically on later runs. Historical fetches run with bounded parallelism; set `PALMSCRIPT_HISTORICAL_FETCH_WORKERS` to tune that worker count, while the default stays conservatively capped at `4` to avoid hammering venue rate limits. Independent date-perturbation reruns and walk-forward segments also use bounded CPU parallelism; set `PALMSCRIPT_BACKTEST_ANALYSIS_WORKERS` to tune those analysis workers, or leave it unset to use all available logical CPUs. Override the cache root with `PALMSCRIPT_HISTORICAL_CACHE_DIR`; otherwise PalmScript uses `$XDG_CACHE_HOME/palmscript/historical` or `$HOME/.cache/palmscript/historical`.
...
Le mode portfolio pilote maintenant aussi le runtime v1 des paniers d'arbitrage. Quand un script declare `arb_entry`, `arb_exit` et `arb_order entry|exit = market_pair(...)`, PalmScript execute une jambe d'achat et une jambe de vente sur les alias spot selectionnes a l'ouverture de la barre suivante. En v1, le premier alias portfolio selectionne sert de runtime controleur, `size = ...` est interprete comme une quantite d'actif de base, et `limit_pair(...)` / `mixed_pair(...)` echouent encore au runtime.

Ce meme runtime controleur evalue maintenant aussi `transfer quote = quote_transfer(...)`. En v1, la quote source est debitee a l'ouverture de la barre suivante et la destination est creditee apres `delay_bars`. `transfer base = base_transfer(...)` reste reserve mais echoue encore au runtime.

Les payloads de resultat orientes backtest resumment maintenant aussi explicitement cette mecanique de portfolio. `run backtest` expose des sections typees `arbitrage` et `transfer_summary`, `run walk-forward` transporte les memes donnees dans les resumes stitches et holdout, et `run optimize --direct-validate-top` renvoie les memes resumes pour chaque replay direct survivant.
...
Les scripts portfolio peuvent aussi classer directement l'alias d'execution
actif pendant l'iteration du portfolio. `current_execution()` expose l'alias
actuellement evalue, tandis que `select_asc(...)`, `select_desc(...)`,
`in_top_n(...)` et `in_bottom_n(...)` comparent les aliases d'execution
selectionnes via leur close d'execution courant :
...
Ce contexte n'existe que pendant l'iteration des aliases d'execution par le
runtime de backtest. Les ordres single-leg peuvent maintenant router
dynamiquement avec n'importe quelle expression `execution_alias`, y compris
`current_execution()` et `select_desc(...)`. A chaque barre, l'expression doit
se resoudre en alias `execution` declare ou en `na`.
...
- le dimensionnement de module peut deja dependre du regime, par exemple `size module breakout = strong ? 0.4 : 0.15`
- entry size expressions are evaluated as hidden numeric series like other order fields
- les ordres en attente conservent la valeur de taille capturee lors de la creation de la demande d'ordre ; ils ne se redimensionnent pas automatiquement a partir des barres futures
