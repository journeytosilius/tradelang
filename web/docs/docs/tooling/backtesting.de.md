...
Trading-Skripte koennen Entry-Rollen auch fuer Modul-Attribution labeln:

- `module breakout = entry long`
- `module pullback = entry2 long`

Diese Labels erscheinen in der Diagnostik als `entry_module` und
`by_entry_module`.
...
- module sizing kann bereits regimebewusst sein, zum Beispiel `size module breakout = strong ? 0.4 : 0.15`
- entry size expressions are evaluated as hidden numeric series like other order fields
- wartende Orders behalten den beim Erzeugen der Order-Anfrage erfassten Size-Wert; sie dimensionieren sich nicht spaeter automatisch aus zukuenftigen Balken neu
